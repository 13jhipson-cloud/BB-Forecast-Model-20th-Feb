...
class Config:
    ...
    # DS rate cap = DS_RATE_CAP_MULTIPLIER x segment historical max event rate
...
def get_cohort_cluster(cohort_ym: int) -> int:
    ...
    FULLY GRANULAR MODEL — ALL backbooks un-clustered (BB1, BB2, BB3, BB4).

    - PRE-2020 (-1) → 201912  (pre-2020 cluster, kept as single unit)
    - 202001+       → INDIVIDUAL (every origination month is its own curve)
    """
    if cohort_ym is None:
        return None
    if cohort_ym == -1:
        return 201912
    # Fully granular: every cohort from 202001 onwards is its own curve
    return cohort_ym
...
def _find_auto_donor(
    ...
    Args:
        ...
        curves_base_df: Historical rate curves (Segment x Cohort x MOB)
        segment: Segment name
...
def build_ds_donor_curves(
    ...
    3. Apply shrinkage:  alpha = min(1, N_own / DS_SHRINKAGE_TARGET)
       blended_rate = alpha x own_rate + (1-alpha) x donor_rate
        ...
        for cohort in cohorts:
            ...
            for event_num, target_mob in enumerate(target_ds_mobs):
                ...
                if event_num < n_own:
                    ...
                    # Applying alpha x 0 + (1-alpha) x donor would wrongly give only
...
def calibrate_ds_scaling(
    ...
    For each calibration month m and segment s:
        actual_m  = sum WO_DebtSold (actuals for that segment/month)
        implied_m = sum OpeningGBV x WO_DebtSold_Rate_DS  (from ds_donor_curves_df)
...
def build_rate_lookup(seed: pd.DataFrame, curves: pd.DataFrame,
    ...
    Args:
        ...
        ds_donor_curves: Pre-built DS donor curves for DSDonorCRScaled approach (optional)
...
def run_one_step(seed_table: pd.DataFrame, rate_lookup: pd.DataFrame,
    ...
    for _, seed_row in seed_table.iterrows():
        ...
        # Layer A: Cohort-level DS curve rate × OpeningGBV = raw pre-scale amount
        # Layer B: Apply segment-level scale factor (WO_DebtSold_ScaleFactor from rate_lookup)
        ...
        output_row = {
            ...
            'WO_DebtSold_Raw': round(wo_debt_sold_raw, 2),          # Pre-scale (Layer A only)
            'WO_DebtSold_ScaleFactor': round(ds_scale_factor, 6),   # Layer B scale factor
...
def generate_comprehensive_transparency_report(
    ...
    Args:
        ...
        ds_donor_map: Optional DS donor curves DataFrame (sheet 18 + 20)
        ds_scale_factors_diag: Optional Layer B calibration diagnostic DataFrame (sheet 19)
...
def run_backbook_forecast(fact_raw_path: str, methodology_path: str,
    ...
    try:
        ...
        # 4b. Build DS donor curves (Layer A) and calibrate segment-level scaling (Layer B).
        # This always runs; ds_donor_curves is only consumed by apply_approach() when the
        # Approach column is 'DSDonorCRScaled'.  For all other approaches, ds_donor_curves
        # is ignored and WO_DebtSold_ScaleFactor defaults to 1.0 (identity).
